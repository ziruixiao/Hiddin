...
- (void)setBadgeValue:(NSInteger)value {
  if(value != 0 || self.displayWhenZero) {
    ...
    self.center = (self.placement == kBadgePlacementUpperLeft) ? CGPointMake(superviewFrame.size.width/2, superviewFrame.size.height/2+diameter+diameter/3) : CGPointMake(superviewFrame.size.width, 0);
  } else {
